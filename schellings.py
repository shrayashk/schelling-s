...
def get_neighbors_internal_nodes(u,v):
	return[(u-1,v),(u+1,v),(u,v-1),(u,v+1),(u-1,v-1),(u-1,v+1),(u+1,v-1),(u+1,v+1)]

def get_neighbors_boundary_nodes(u,v):
	if u==0 and v==0:
		return[(0,1), (1,1), (1,0)] 
	...
	for u,v in G.nodes():
		...
		if type_of_this_node==0:
			...
			if (u,v) in internal_nodes_list:
				neigh=get_neighbors_internal_nodes(u,v)
			elif (u,v) in boundary_nodes_list:
				neigh=get_neighbors_boundary_nodes(u,v)
			for each in neigh:
...
def satisfy(unsatisfied_nodes_list,empty_cells):
	if len(unsatisfied_nodes_list) !=0:
		node_to_shift=random.choice(unsatisfied_nodes_list)
		new_posotion=random.choice(empty_cells)

		G.node[new_posotion]['type']=G.node[node_to_shift]['type']
		G.node[node_to_shift]['type']=0
		labels[node_to_shift],labels[new_posotion]=labels[new_posotion],labels[node_to_shift]
	else :
		pass
...
for i in range (9999):
	...
	satisfy(unsatisfied_nodes_list,empty_cells)
	type_1_node_list=[n for (n,d) in G.nodes(data=True) if d['type']==1]
	type_2_node_list=[n for (n,d) in G.nodes(data=True) if d['type']==2]
	empty_cells=[n for (n,d) in G.nodes(data=True) if d['type']==0]
